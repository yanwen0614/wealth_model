...
|---|---|---|---|---|
| T01 | 定义批准特征契约 | pending | - | 显式 39 raw -> 45 output |
| T02 | 添加 scaler identity 与 schema 校验 | pending | - | v3 payload |
| T03 | 修正变换、缺失与 fallback | pending | - | 共享语义 |
| T04 | 强制缓存策略与验证 context 边界 | pending | - | train-only fit |
| T05 | 打通已验证的训练/评估复用 | pending | - | eval training identity |
| T06 | 更新规范并执行回归测试 | pending | - | synthetic fixtures |
