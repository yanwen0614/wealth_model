...
|------|------|------|--------------|------|
| T01 | uv.lock 非法字符修复 | pending | - | 删 148 行多余 `]`，TOML 解析验证 |
| T02 | CLI 重构为 adapter 唯一路径 | pending | - | 依赖 T01；重写 run_backtest.py 自洽 |
| T03 | adapter target 外层策略类 | pending | - | 依赖 T02（接口约定）；order_strategy.py |
| T04 | runner 费用透传 + 策略选择 | pending | - | 依赖 T03；runner.py 扩展 |
| T05 | CLI 指数基准 + 费用/废弃参数 | pending | - | 依赖 T04；与 T03 可部分并行 |
| T06 | run_eval_full.sh Step2 切换 | pending | - | 依赖 T02；独立小改 |
| T07 | 单测改/留对齐 | pending | - | 依赖 T02–T06；N01/N02/CLI 改，engine 留 |
...
- **依赖**：T02
- **文件**：
  - `backtest/cnn_adapter/order_strategy.py` (modify，+~120 行)
- **预估行数**：+120
